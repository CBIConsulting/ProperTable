...
var ProperTable = function (_React$Component) {
   ...
   */
		...
		value: function applySettings() {
			...
			var selectionSet = {},
			    ...
			    hasSort = false,
			    newCol = undefined;
			var updateSortAllowed = updateSort && props.colSortDirs && props.colSortDirs.length > 0;
			var updateFiltersAllowed = updateFilters && props.colFilters,
			    hasSelectionFilter = undefined,
			    hasCustomFilter = undefined,
			    operations = {};
				...
				if (updateFiltersAllowed) {
					if (props.colFilters[col.column]) {
						...
						if (!newFilter.type || newFilter.type === FILTERTYPE_SELECTION) {
							...
							if (!hasFilter && col.filterType !== FILTERTYPE_SELECTION) hasFilter = true;else if (!hasFilter) hasFilter = !(0, _reactImmutableRenderMixin.shallowEqualImmutable)(col.selection, newFilter.selection);
						...
						columns.every(function (column) {
							...
							if (typeof val === 'string' || typeof val === 'number') {
								...
								if (filters[column]) {
									...
									result = _this3.customFilter(operations[column].type, operations[column].value, val);
								}
							}
   ...
   */
		...
		value: function customFilter(type, value, compareTo) {
			return _comparators2['default'][type](value.toString().toLowerCase(), compareTo.toString().toLowerCase());
		}
   ...
   */
		...
		value: function clearFilterAndSort(e) {
			...
			data = this.state.initialData.map(function (element, index) {
				if (_this5.state.selection.has(element.get(_this5.props.idField))) {
					element = element.set(SELECTED_FIELD, true);
				}
				indexed[element.get(_this5.props.idField)]._rowIndex = index; // Update index into indexed data.
   ...
   */
		...
		value: function sortColumns(data, indexed, colSettings) {
			...
			sortedData.map(function (element, index) {
				indexedData[element.get(_this6.props.idField)]._rowIndex = index;
			});
